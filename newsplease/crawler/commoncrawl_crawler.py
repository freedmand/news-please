...
desal_re = re.compile("desal(\s|i|$)", re.IGNORECASE)


class DesalCommonCrawlExtractor(CommonCrawlExtractor):
    def filter_record(self, warc_record, article=None):
        ...
        if article is None:
            article = self._from_warc(warc_record, lambda text: desal_re.search(text))
        if article is None:
            return False, None
        if article.maintext is None:
            return False, article
        if desal_re.search(article.maintext):
            return True, article
        return False, article
...
def __start_commoncrawl_extractor(
    ...
    extractor_cls=DesalCommonCrawlExtractor,
    fetch_images=False,
):
    """
